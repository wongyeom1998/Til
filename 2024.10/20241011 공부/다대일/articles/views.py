...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # DB에는 반영되진 않았지만,, 사용자가 form으로 보낸
            # title, content를 포함한 article 객체가 만들어졌고,
            # 그 article 객체는 ArticleForm으로 만들어졌으니,
                # class Meta의 model = Article을 적어 놨기 때문에
            # 내가만든 models의 Article class의 인스턴스가 되겠다.
            # 그래서 article.user 정보에 요정보낸 user의 정보를 담는다.
